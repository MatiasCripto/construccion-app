// src/components/AdminPanel.js - VERSIÓN COMPLETA CON FUNCIÓN ELIMINAR
...
const AdminPanel = () => {
  ...
  // ===== NUEVOS ESTADOS PARA ELIMINAR USUARIO =====
  ...
  const confirmDeleteUser = async () => {
    ...
    try {
      ...
      // 1. Eliminar del backend (API REST)
      ...
      // 2. Eliminar datos de Firebase (si existe)
      if (window.db && window.FirebaseService) {
        console.log('📍 Eliminando ubicaciones del usuario...');
        try {
        ...
        console.log('💬 Eliminando mensajes del usuario...');
        try {
          const messagesSnapshot = await window.db.collection('mensajes')
            .where('autorId', '==', userToDelete.id)
            .get();
        ...
        console.log('👤 Eliminando usuario de Firebase...');
        try {
